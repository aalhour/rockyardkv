...
### What is a “C++ corpus root”?

A **C++ corpus root** is **just a directory on your machine** that contains a collection (“corpus”) of **complete RocksDB database directories** that were **generated by C++ RocksDB**.

RockyardKV uses this corpus to continuously answer the question:

- “Can Go open and read real C++-generated on-disk data across many formats/options?”

This corpus is intentionally **external** (not checked into this repo) because:

- it can be large,
- it may grow over time as new fixtures are added, and
- teams may generate and share corpora out-of-band.

### Set up the corpus

Export the `ROCKYARDKV_CPP_CORPUS_ROOT` environment variable:

```bash
export ROCKYARDKV_CPP_CORPUS_ROOT=/path/to/corpus
```
...
The corpus tests look up the root directory via:

- `ROCKYARDKV_CPP_CORPUS_ROOT`

Then each test opens one of the expected subdirectories under that root (for example, `zlib_small_blocks_db/db/`) and performs a read-only validation in Go.
If the directory is missing, the test **skips** (so regular CI isn’t forced to carry the full external corpus).
...
1. `ROCKYARDKV_CPP_CORPUS_ROOT` is set and points to the corpus directory.
