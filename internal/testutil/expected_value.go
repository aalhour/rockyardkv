...
func (es *ExpectedStateV2) SaveToFile(path string) error {
	...
	// Data
	for i := range es.values {
		// Preserve pending flags exactly as they are.
		// During verification, keys with pending flags are SKIPPED because we don't know
		// if the in-flight operation completed before the crash.
		// This prevents both false positives and false negatives:
		// - If we assumed pending writes complete and they don't -> false positive
		// - If we assumed pending deletes complete and they don't -> false positive
		// By preserving the flags, verification skips uncertain keys.
		v := es.values[i].Load()
