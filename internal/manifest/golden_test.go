...
func TestGoldenManifestSimple(t *testing.T) {
	goldenPath := filepath.Join("..", "..", "testdata", "rocksdb", "v10.7.5", "formats", "manifest", "simple.manifest")
...
func TestGoldenManifestNewFile(t *testing.T) {
	goldenPath := filepath.Join("..", "..", "testdata", "rocksdb", "v10.7.5", "formats", "manifest", "newfile.manifest")
	data, err := os.ReadFile(goldenPath)
	if err != nil {
		t.Skipf("Golden file not found: %v (run golden fixture generation first)", err)
	}
