// go_cpp_golden_test.go - Golden tests that verify Go-written SST files can be read by C++ RocksDB.
//
// These tests:
// 1. Write SST files using Go implementation
// 2. Use C++ sst_dump tool to verify they can be read
// 3. Compare the output to expected values
//
// Prerequisites:
// - C++ RocksDB built with: make sst_dump -j$(nproc)
// - ROCKSDB_PATH environment variable set to ~/Workspace/rocksdb
package table
...
func TestGoWritesCppReads_Simple(t *testing.T) {
	...
	// Write SST using Go
	writeTestSST(t, sstPath, []keyValue{
		{"apple", "red"},
		{"banana", "yellow"},
		{"cherry", "red"},
	})
...
func TestGoWritesCppReads_BinaryKeys(t *testing.T) {
	...
	output := runSstDump(t, sstDump, sstPath)
	if output == "" {
...
func writeTestSST(t *testing.T, path string, kvs []keyValue) {
	t.Helper()
	writeTestSSTWithBlockSize(t, path, kvs, 0)
}

func writeTestSSTWithBlockSize(t *testing.T, path string, kvs []keyValue, blockSize int) {
	t.Helper()
...
func runSstDump(t *testing.T, sstDump, sstPath string) string {
	t.Helper()
...
func TestGoWritesCppReads_RoundTrip(t *testing.T) {
	...
	for i, k := range keys {
		kvs[i] = keyValue{key: k, value: testData[k]}
	}
...
func TestGoWritesCppReads_EdgeCases(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "single_entry",
			kvs:  []keyValue{{"only_key", "only_value"}},
		},
		{
			name: "two_entries",
			kvs:  []keyValue{{"first", "1"}, {"second", "2"}},
		},
		{
			name: "long_key",
			kvs:  []keyValue{{strings.Repeat("k", 1000), "value"}},
		},
		{
			name: "special_chars",
			kvs:  []keyValue{{"key with spaces", "value\twith\ttabs"}, {"key:colon", "value;semicolon"}},
		},
	}
...
func TestCppWritesGoReads_GoldenFiles(t *testing.T) {
	...
	goldenPath := "../../testdata/golden/v10.7.5/sst/simple.sst"

	if _, err := os.Stat(goldenPath); os.IsNotExist(err) {
		t.Skip("Golden SST file not found")
	}

	fs := vfs.Default()
	file, err := fs.OpenRandomAccess(goldenPath)
	if err != nil {
		t.Fatalf("Failed to open golden SST: %v", err)
	}
	...
	for iter.Valid() {
		count++
		t.Logf("Entry %d: key=%x, value=%s", count, iter.Key(), iter.Value())
		iter.Next()
	}
	...
	if count == 0 {
		t.Error("No entries found in golden SST")
	}
}
...
func TestBidirectionalCompatibility(t *testing.T) {
	...
	t.Run("GoWritesCppReadsGoReads", func(t *testing.T) {
		...
		writeTestSST(t, sstPath, originalData)

		// C++ reads
		cppOutput := runSstDumpScan(t, sstDump, sstPath)

		// Go reads
		fs := vfs.Default()
		file, _ := fs.OpenRandomAccess(sstPath)
		reader, _ := Open(file, ReaderOptions{VerifyChecksums: true})
		defer reader.Close()
